    ...
    <form action={{ route('game_master.update','game_master' ) }} method="POST" class="d-flex row p-4
            ...
            <div class="col-6 mb-3">
                ...
                <select class="form-select" name="location" id="location">
                    ...
                    <option value="{{ $single_province }}" @if (old('location')==$single_province || ($game_master &&
                        $game_master->location == $single_province))
                        selected
                        @endif>
                        {{ $single_province }}
                    </option>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" name="game_description" id="game_description"
                rows="3">{{ old('game_description' , $game_master->game_description) }}</textarea>
        </div>
        <div class="mb-3">
            {{-- @dd($game_systems) --}}
            ...
            <div class="form-check">
                <input name="game_systems[]" class="form-check-input" id="game_system-{{ $game_system->id }}"
                    ...
                    $game_master->gameSystems->contains($game_system->id) || in_array($game_system->id, (array)
                old('game_systems', [])) ? 'checked' : '' }}>
